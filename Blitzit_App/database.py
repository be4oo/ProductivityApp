...
def update_task_column(task_id, new_column):
    conn = get_db_connection()
    operation_details = {"recurred": False, "title": None}
    ...
    if new_column == "Done":
        ...
        if not task:
            conn.close()
            return operation_details # Task not found

        operation_details["title"] = task['title']
        ...
        if task['recurrence']:
            ...
            operation_details["recurred"] = True # Mark that recurrence happened
    else:
